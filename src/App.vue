  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">提问领域</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">回答调性</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">知乎高赞回答写作与知乎体风格指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 知乎爆款回答与深度逻辑专家');
...
const presetOptions = [
  "“如何看待当代年轻人选择躺平”的高赞硬核回答",
  "人工智能 AI 大模型发展是否会导致大规模失业",
  "从博弈论角度深度解构职场向上管理与生存法则",
  "“为什么越穷的人越容易陷入死循环”心理学回答"
];
...
const metricsList = [
  {
    "key": "zhihuStyleProfessionalism",
    "label": "知乎硬核专业度"
  },
  {
    "key": "upvoteIncentiveAttraction",
    "label": "高赞赞同吸引力"
  },
  {
    "key": "logicalArgumentDepth",
    "label": "逻辑论证严密深度"
  },
  {
    "key": "counterIntuitiveInsight",
    "label": "反直觉洞察分"
  },
  {
    "key": "communityDiscussionVirality",
    "label": "盐选社区讨论潜能"
  }
];
const guideRules = [
  {
    "title": "开篇抛出反直觉结论",
    "advice": "知乎读者喜欢看“先说结论”和打破固有思维的视角（如“大多数人都想错了，真相其实是…”）",
    "avoid": "切忌平铺直叙像教科书一样死板"
  },
  {
    "title": "论据硬核引用论文数据",
    "advice": "回答中引用具体实验研究、学者著作或权威论文数据，大幅提升回答说服力",
    "avoid": "避免凭空感觉或只有主观情绪宣泄"
  },
  {
    "title": "知乎体排版留白舒适",
    "advice": "多用加粗核心结论、引言引用框（引用格言）及有序列表，提高长文可读性",
    "avoid": "严禁一大段无分段的文字堆砌"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[ZHIHU_SCORES\][\s\S]*?\[\/ZHIHU_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[ZHIHU_SCORES\](.*?)\[\/ZHIHU_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('zhihu_history_records');
...
const saveHistory = () => {
  localStorage.setItem('zhihu_history_records', JSON.stringify(historyList.value));
};
